...
StyleDictionaryPackage.registerFormat({
  ...
  format: async ({ dictionary, file }) => {
    const tokenObj = dictionary.tokens;
    return (await fileHeader({ file })) + `export const componentTokens = ` + JSON.stringify(minifyDictionary(tokenObj));
  },
});
...
StyleDictionaryPackage.registerFormat({
  ...
  format: async ({ dictionary, file }) => {
    const tokenObj = dictionary.tokens;
    return (await fileHeader({ file })) + `export const componentConfig = ` + JSON.stringify(minifyDictionary(tokenObj));
  },
});
...
themes.array.map(async theme => {
  console.log('\n==============================================');
  console.log(`\nStyleDictionary Processing: [${theme}]`);

  const StyleDictionary = await StyleDictionaryPackage.extend(getStyleDictionaryConfig(theme));
